...
def generate_launch_description():
    laser_sensor_name = os.getenv('ROS2_DIFF_DRIVE_ROBOT_LASER_SENSOR', '')
    base_laser_sensor_name = os.getenv('ROS2_DIFF_DRIVE_ROBOT_BASE_LASER_SENSOR', '')
    depth_sensor_name = os.getenv('ROS2_DIFF_DRIVE_ROBOT_DEPTH_SENSOR', '')
    
    fake_laser_config_path = PathJoinSubstitution(
        [FindPackageShare('ros2_diff_drive_robot_bringup'), 'config', 'fake_laser.yaml']
    )
    ...
    laser_launch_path = PathJoinSubstitution(
        [FindPackageShare('ros2_diff_drive_robot_bringup'), 'launch', 'lasers.launch.py']
    )

    depth_launch_path = PathJoinSubstitution(
        [FindPackageShare('ros2_diff_drive_robot_bringup'), 'launch', 'depth.launch.py']
    )
